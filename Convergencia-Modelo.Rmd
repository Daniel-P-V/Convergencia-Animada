...
```{r, include = FALSE}
  ...
  plot1 <- ggplot(data = d1, aes()) +
            ...
            scale_colour_manual(values = c("orangered3", 
                                           "blue4", 
                                            "#66CC99")) +
            geom_vline(xintercept = ke,
                       linetype = 'dashed') +
            geom_hline(yintercept = ye,
                       linetype = 'dashed') +
...
Convergencia desde un valor de $k=0.5$ hasta el Estado Estacionario

```{r, echo = FALSE}
  # Animation 
  animacion <- plot1 + geom_point(data = d2,
                                  aes(x = kee, y = y, group = Puntos),
                                  color = "red",
                                  size = 3) +
                        transition_states(kee,
                                          transition_length = 2,
                                          state_length = 1) +
